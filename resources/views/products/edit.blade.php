...
<body class="max-w-7xl mx-auto p-4">
    ...
    <form action="/products/{{ $product->id }}" method="POST" class="border p-4">
        @csrf
        @method('PUT')
        
        <div class="mb-4">
            <label for="nom" class="block mb-1">Name:</label>
            <input type="text" name="nom" id="nom" value="{{ $product->nom }}" class="border p-2 w-full" required>
        </div>

        <div class="mb-4">
            <label for="reference" class="block mb-1">Reference:</label>
            <input type="text" name="reference" id="reference" value="{{ $product->reference }}" class="border p-2 w-full" required>
        </div>

        <div class="mb-4">
            <label for="description_courte" class="block mb-1">Description:</label>
            <textarea name="description_courte" id="description_courte" class="border p-2 w-full" rows="3">{{ $product->description_courte }}</textarea>
        </div>

        <div class="mb-4">
            <label for="prix" class="block mb-1">Price:</label>
            <input type="number" step="0.01" name="prix" id="prix" value="{{ $product->prix }}" class="border p-2 w-full" required>
        </div>

        <div class="mb-4">
            <label for="stock" class="block mb-1">Stock:</label>
            <input type="number" name="stock" id="stock" value="{{ $product->stock }}" class="border p-2 w-full" required>
        </div>
        ...
        <div class="mb-4">
            ...
            <input type="url" name="image" id="image" value="{{ $product->image }}" class="border p-2 w-full" placeholder="https://example.com/image.jpg">
        </div>
